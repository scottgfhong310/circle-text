...
I18n.register('ja', {
  ...
  /* 外徑上限・紙張・版面反解 */
  'param.outerMax': '外径の上限 (mm)',
  'param.paper': '用紙',
  'param.paperMargin': '片側余白 (mm)',
  'paper.custom': 'カスタム',
  'dims.fits': '収まります（上限 {max} mm）',
  'dims.overLimit': '上限 {max} mm を超過',
  'warn.overWidth': '外径 {outer} mm が上限 {max} mm を {over} mm 超えています——収まりません。',
  'err.paperMargin': '余白が大きすぎて、この用紙には使える幅が残りません。',
  'tool.plan': '外径からパラメータを逆算',
  'plan.title': '外径からパラメータを逆算',
  'plan.intro': '<strong>総文字数</strong>・<strong>外径の上限</strong>・<strong>内径</strong>を決めると、文字サイズとリング数は厳密に比例します——R<sub>1</sub> + R<sub>o</sub> は内外径だけで決まり、文字サイズは総文字数の式から消えるからです。したがってこの制約の解は<strong>一つではなく一族</strong>：下表はどの行も成立し、違いは行間の詰まり具合だけです。',
  'plan.note': '外径と総文字数は<strong>固定</strong>（用紙をはみ出さない・文字が全部入る）。リング数と内側文字数は整数でなければならないため、<strong>内径</strong>がわずかに譲ります——括弧内は設定値との差です。行間比＝リング間隔 ÷ 文字サイズ、1 未満なら隣り合うリングの文字が縦に重なります。',
  'plan.innerTarget': '中空部の直径 (mm)',
  'plan.rings': 'リング数',
  'plan.fontSize': '文字サイズ',
  'plan.gap': '間隔 (pt)',
  'plan.ratio': '行間比',
  'plan.n1': '内側文字数',
  'plan.inner': '実際の内径 (mm)',
  'plan.hidden': '他に {n} 件の解は未掲載です——行間比が {lo}–{hi} の範囲外（詰まりすぎると重なり、緩すぎると文字が小さすぎて意味がありません）。',
  'plan.apply': '適用',
  'plan.cTotal': '総文字数',
  'plan.cOuter': '外径の上限',
  'plan.cInner': '中空部の直径',
  'planErr.badTotal': '先に総文字数を入力してください。',
  'planErr.badOuterMax': '先に外径の上限を入力してください（用紙から換算も可）。',
  'planErr.badInnerTarget': '先に内径を入力してください。',
  'planErr.innerGeOuter': '内径は外径の上限より小さくしてください。',
  'planErr.noCandidate': 'この制約では成立する版面がありません。外径を広げるか内径を小さくしてください。',
  'toast.planApplied': '{n} リングの案を適用しました',
